...
8. ~~Foundry UI extension/customization~~ (ADAPT-003, issue #20) — implemented (`src/adapters/foundry/actor-node-type-tag.ts`, 2026-08-31, `decisions/ADR-0009-foundry-ui-extension-hooks.md`): a `flags.archivexus.nodeType` tagging control injected into the Actor sheet via `renderActorSheetV2`, replacing the console-only workflow. `buildNodeTypeTagHTML` is pure/unit-tested (12 cases); insertion point and `submitOnChange` behavior verified against a real dnd5e2 `CharacterActorSheet` render in Alberto's live campaign world (`ADR-0009`'s addendum) before writing any code. Scoped to Actor first, matching every prior Adapter ticket's incremental-per-document-type shape; JournalEntry/Scene tagging UI and any `getHeaderControls*` actions are separate, not-yet-scoped follow-up work. Not yet built into a release or reloaded in Foundry — `npm run build:foundry-module` and a live visual check are still needed from Alberto, same as every prior Foundry-side change.

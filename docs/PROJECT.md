...
3. ~~Implement the Foundry Adapter's `JournalEntryPage` → Node mapping~~ — done 2026-08-30 (`src/adapters/foundry/journal-entry-page-to-node.ts`, ADAPT-001). Type-checked and functionally verified (`npm install`/`npm test` still pending real registry access this session). Board still shows ADAPT-001 as "Ready for Implementation" — move it once reviewed/merged. Deliberately doesn't yet map page content to Blocks (`Block`'s shape is still a placeholder) or resolve Foundry's ownership-inheritance sentinel (needs a live Foundry instance) — both flagged as open follow-ups, not yet their own tickets.

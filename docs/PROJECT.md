...
9. Relationship (Core domain, per ADR-0005) — still held by the user until real example data exists from the Foundry integration work above, but no longer blocked on an undefined shape: `decisions/ADR-0007-relationship-view-traversal.md` (2026-08-30) settled how a View selects which connected Relationships/Nodes to include (category-tagged Relationship Definitions + a Core Query API), so Relationship Definition's schema now has a concrete target (a `traversalCategory` field) to implement against once this starts. Then View, lower priority still since it depends on having enough Nodes/Relationships worth projecting; its own MVP scope (3 fixed traversal presets, no open config) is also now settled by the same ADR.

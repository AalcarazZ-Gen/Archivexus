...
8. Foundry UI extension/customization (ADAPT-003, issue #20) — mechanism decided (`decisions/ADR-0009-foundry-ui-extension-hooks.md`, 2026-08-31): native `render*`/`getHeaderControls*` Hooks only, no `libWrapper`, no sheet subclassing/replacement. First concrete surface: a `flags.archivexus.nodeType` tagging control injected into the Actor sheet via `renderActorSheetV2`, replacing the current console-only workflow. Scoped to Actor first, matching every prior Adapter ticket's incremental-per-document-type shape; JournalEntry/Scene tagging UI and any `getHeaderControls*` actions are separate, not-yet-scoped follow-up work. Research-only pass, per the issue's own scoping — implementation (the actual injected `<select>`/form section) hasn't started.

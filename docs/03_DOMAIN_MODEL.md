...
Only when the GM explicitly tags it (ADR-0011 Amendment 2, 2026-09-08). Default (untagged): no — each page maps individually.

A Foundry `JournalEntry` is a Foundry-native container of `JournalEntryPage` documents. **By default** the container itself is a storage detail, not an Archivexus domain concept (`01_ARCHITECTURE.md`'s "Knowledge over Documents" principle): the Foundry Adapter maps each page with distinct semantic content to its own Node, using that page's own Foundry UUID per ADR-0001 — **unless the GM has explicitly tagged the page as attached to another Node (`flags.archivexus.attachedToNodeId`), in which case it becomes a Block on that Node instead of a Node of its own; see `decisions/ADR-0011-journal-entry-page-node-attachment.md` for the full mechanism, storage shape and migration behavior. Amended by ADR-0011, 2026-09-06.** The Node's type comes from an explicit GM-set flag, never inferred from content. Pages without an explicit type become a generic `Lore` Node.

**But** — since ADR-0015 established that a Foundry container a GM has organised their world around (a Folder) becomes a Node on an explicit opt-in tag, **ADR-0011 Amendment 2 (2026-09-08)** applies the same to a `JournalEntry`: a GM can tag a whole entry (`flags.archivexus.nodeType` on the entry, set from the journal directory context menu) → it becomes **one** Node (`id = JournalEntry.<id>`, `title = entry.name`, visibility from `entry.ownership.default`), and its pages become `{ type: 'JournalEntryPage', uuid, title }` Blocks on it (an engine-owned array, reconciled wholesale). While the entry is tagged, its pages are not their own Nodes and their individual flags are not read (the entry-tag supersedes). The entry-Node participates in folder-containment like a tagged Actor. This is the ergonomic answer to "a member of an organization is a multi-page character" — one tag, not tag-one-page-and-attach-the-rest. See `decisions/ADR-0011-journal-entry-page-node-attachment.md`'s Amendment 2 in full.
